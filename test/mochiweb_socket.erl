...
finish_accept(#ssl_socket{tcp = Socket}) ->
    case ssl:ssl_accept(Socket, ?SSL_HANDSHAKE_TIMEOUT) of
        ok ->
            {ok, #ssl_socket{ssl = Socket}};
        {error, _} = Err ->
            Err
    end;
finish_accept(Socket) ->
...
recv(#ssl_socket{ssl = Socket}, Length, Timeout) ->
    ssl:recv(Socket, Length, Timeout);
...
send(#ssl_socket{ssl = Socket}, Data) ->
    ssl:send(Socket, Data);
send(Socket, Data) ->
...
peername(#ssl_socket{ssl = Socket}) ->
    ssl:peername(Socket);
peername(Socket) ->
    inet:peername(Socket).

setopts(#ssl_socket{ssl = Socket}, Opts) ->
    ssl:setopts(Socket, Opts);
setopts(Socket, Opts) ->
    inet:setopts(Socket, Opts).

getopts(#ssl_socket{ssl = Socket}, Opts) ->
